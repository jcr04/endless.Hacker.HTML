...
const gameFuncs = {
    ...
    drawGame: function(wds) {
        ...
        ctx.fillText('Vidas: ', canvasData.cWidth - 130, canvasData.cHeight - 6);
    ...
    gameOverInfo: function() {
        ...
        canvasData.ctx.fillText('Pressione \'R\' pra tentar novamente', (canvasData.cWidth / 2) - 160, 210);
        canvasData.ctx.fillText('Você conseguiu marcar: ' + gameFuncs.score, (canvasData.cWidth / 2) - 220, 300);
    },
    ...
    gameIntro: function() {
        ...
        canvasData.ctx.fillText('Oh não! Você foi hackeado e colocado offline.', 10, 60);
        canvasData.ctx.fillText('Que hacker nojento e diabolicamente bonito fez isso?', 10, 90);
        canvasData.ctx.fillText('Bem, você nunca descobrirá, pois esta é a única tela da história.', 10, 120);
        canvasData.ctx.fillText('A única maneira de tentar voltar a ficar online é interromper o hack.', 10, 170);
        canvasData.ctx.fillText('Para fazer isso você terá que inserir as palavras que aparecem antes', 10, 200);
        canvasData.ctx.fillText('eles alcançam o lado direito da tela.', 10, 230);
        canvasData.ctx.fillText('Por algum motivo, isso retardará o hack. Não questione', 10, 280);
        canvasData.ctx.fillText('este poderoso herói, suas habilidades de digitação são necessárias. Boa sorte!', 10, 310);

        canvasData.ctx.fillRect(0, 370, canvasData.cWidth, 48)

        canvasData.ctx.font="bold 32px monospace";
        canvasData.ctx.fillStyle="black";
        canvasData.ctx.fillText('Para começar pressione \'Enter\' ou \'R\'', 18, 405);
    },
    gameEngine: function() {
